...
%>
%> This command is the same as MODBUSREGINT16 in AeroBasic.
%>
%> @param handle The handle to the controller on which to execute the command
%> @param taskId The ID of the task on which to execute the command
%> @param Axis Specify the name of the axis on which to use the Modbus register set.
%> @param RegName The register set to access (MasterInputWords, SlaveInputWords, MasterOutputWords, or SlaveOutputWords). The input words have read-only access, and the output words have read and write access.
...
%> @ingroup Modbus
function [returnValue] = A3200ModbusRegisterInt16ReadDrive(handle, taskId, Axis, RegName, RegNumber)
  ...
  [returnValue] = A3200ModbusRegisterInt16ReadDrive_internal(handle, taskId, Axis, RegName, RegNumber);
